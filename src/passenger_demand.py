...
def visualize_demand(demand_array, airports):
    ...
    summarized_demand = np.sum(demand_array, axis=2)  # sum over the third dimension (days)
    
    sns.heatmap(summarized_demand, xticklabels=airport_ids, yticklabels=airport_ids, annot=True, cmap='YlGnBu')
...
def visualize_demand_for_day(demand_array, airports, day):
    ...
    sns.heatmap(day_demand, xticklabels=airport_ids, yticklabels=airport_ids, annot=True, cmap='YlGnBu')
    plt.xlabel('To Airport')
    plt.ylabel('From Airport')
    plt.title('Summarized Passenger Demand Heatmap')
    plt.show()
